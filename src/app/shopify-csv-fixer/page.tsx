    ...
    <div className="mx-auto max-w-5xl px-6 py-12">
      <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-sm">
        ...
        <p className="mt-3 text-base text-[color:rgba(var(--muted-rgb),1)]">
          Upload your Shopify Products CSV and we’ll highlight exactly what Shopify will reject — then auto-fix the
          safe stuff (booleans, pricing formats, inventory rules, schema issues) so your import works the first time.
        </p>
      ...
      <section className="mt-10 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-8">
        ...
        <div className="mt-4 space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[color:rgba(var(--muted-rgb),1)]">
              We auto-fix deterministic issues (formatting, normalization, missing headers). Anything that requires
              business context (like choosing the correct handle, title, or vendor) is left for manual editing.
            </p>
          </div>
          <div>
            <div className="text-sm font-semibold text-[var(--text)]">Do you support WooCommerce, Etsy, eBay…?</div>
            <p className="mt-1 text-sm text-[color:rgba(var(--muted-rgb),1)]">
              Yes. StriveFormats is built around presets. Shopify is the most advanced today, and the same framework
              scales to 20+ formats.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <Link href="/formats/presets" className="text-sm font-semibold text-[var(--text)] hover:underline">
            See all presets
          </Link>
        </div>
      </section>
      ...
      <div className="mt-10 text-sm text-[color:rgba(var(--muted-rgb),1)]">
        ...
        <Link className="hover:underline" href="/formats/presets">
          Preset Formats
        </Link>
      </div>
    </div>
